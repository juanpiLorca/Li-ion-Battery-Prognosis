...
class UnscentedKalmanFilter:
    ...
    def __init__(self, nx, ny): 
        ...
        # UKF parameters: 
        self._alpha = 1e-3
        self._beta = 2.0         # optimal for Gaussian distributions
        self._kappa = 0.0        
        
        self._lambda = (self._alpha ** 2) * (self.nx + self._kappa) - self.nx
        self._gamma = np.sqrt(self.nx + self._lambda)

        self.W0m = self._lambda / (self.nx + self._lambda)
        self.W0c = self._lambda / (self.nx + self._lambda) + (1.0 - self._alpha ** 2 + self._beta)
        self.Wim = 1.0 / (2.0 * (self.nx + self._lambda))
        self.Wic = 1.0 / (2.0 * (self.nx + self._lambda))

        # State variables used in the UKF process - buffers 
        self.x_apriori = np.zeros((self.nx,), dtype=float)
        self.x_aposteriori = np.zeros((self.nx,), dtype=float)
        self.y = np.zeros((self.ny,), dtype=float)
        self.x_P = np.zeros((self.nx,), dtype=float)
        self.y_P = np.zeros((self.ny,), dtype=float)

        # Covariance matrices used in the process
        self.P_apriori = np.zeros((self.nx, self.nx), dtype=float)
        self.P_aprioriP = np.zeros((self.nx, self.nx), dtype=float)
        self.P_aposteriori = np.zeros((self.nx, self.nx), dtype=float)

        # Square root product of a given covariance
        self.sqrtP_apriori = np.zeros((self.nx, self.nx), dtype=float)
        self.sqrtP_aposteriori = np.zeros((self.nx, self.nx), dtype=float)

        # Clear sigma points
        self.x_sigma = np.zeros((self.nx, (2 * self.ny + 1)), dtype=float)
        self.y_sigma = np.zeros((self.ny, (2 * self.nx + 1)), dtype=float)

        # Cross covariances
        self.P_xy = np.zeros((self.nx, self.ny), dtype=float)
        self.P_xyP = np.zeros((self.nx, self.ny), dtype=float)

        # Output covariance
        self.P_y = np.zeros((self.ny, self.ny), dtype=float)
        self.P_yP = np.zeros((self.ny, self.ny), dtype=float)

        # Kalman gain
        self.K = np.zeros((self.nx, self.ny), dtype=float)

        # Process and measurement noise covariance matrices
        self.Q = np.zeros((self.nx, self.nx), dtype=float)
        self.R = np.zeros((self.ny, self.ny), dtype=float)

    def resetUKF(self, Q, R):
        """
        Reset the UKF with new process noise covariance Q and measurement noise covariance R.
        ...
        self.x_apriori.fill(0)
        self.x_aposteriori.fill(0)
        self.x_P.fill(0)
        self.y.fill(0)
        self.y_P.fill(0)

        # Reset covariance matrices
        self.P_apriori = self.Q.copy()  
        self.P_aposteriori = self.Q.copy()
        self.P_aprioriP.fill(0)
    ...
    def computeSigmaPoints(self, x, P):
        """
        ...
        for i in range(n):
            sigma_points[:, i + 1] = x + self._gamma * sqrt_P[:, i]
            sigma_points[:, i + 1 + n] = x - self._gamma * sqrt_P[:, i]
